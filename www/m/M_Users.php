...
{	
	...
	public function Login($mail, $password, $remember = true)
	{
		...
		if ($user != null)
			return false;
                ...
                $mail_s = $this->msql->real_escape_string($mail);
                $password_hash = password_hash($password, PASSWORD_BCRYPT);
            
                $result = $this->msql->Insert('users',array('mail'=>"$mail_s", 'password'=>"$password_hash"));
                return $result;
        }
	...
	{	
		...
		$query = sprintf($t, $this->msql->real_escape_string($mail));
		$result = $this->msql->Select($query);
		return $result[0];
	}
	...
	{	
		...
		$query = sprintf($t, $this->msql->real_escape_string($sid));
		$result = $this->msql->Select($query);
	...
	private function GetSid()
	{
		...
		if ($sid != null)
		{
			...
			$t = "sid = '%s'";
			$where = sprintf($t, $this->msql->real_escape_string($sid));
